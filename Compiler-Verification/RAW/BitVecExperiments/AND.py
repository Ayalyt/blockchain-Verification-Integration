...
from BitVecExperiments.FrFunctionModels.AND_parser import ANDCircuitConstraintParser
from FrFunctionModels.XOR_parser import *


def run_AND_constraint(p):
    s = Solver()
    bw = p.bit_length() * 2
    ...
    circom_constraint = out % p == (a * b) % p
    R1CS_constraint = ((a * -1) % p * (b * 1) % p) % p == (out * -1) % p
...
def run_AND_calculation(p):
    s = Solver()
    a = Int('s_a')
    b = Int('s_b')
    out = Int('s_out')
    circom_calculation = out == (a * b) % p

    cpp_parser = ANDCircuitConstraintParser()
