...
from BitVecExperiments.FrFunctionModels.NOR_parser import NORCircuitConstraintParser
from FrFunctionModels.XOR_parser import *


def run_NOR_constraint(p):
    s = Solver()
    bw = p.bit_length() * 2
    ...
    circom_constraint = out % p == (((((a * b) % p + 1) % p - a) % p) - b) % p
    R1CS_constraint = ((a * m1) % p * (b * 1) % p) % p == (1 + (m1 * out) % p + (m1 * a) % p + (m1 * b) % p) % p
...
def run_NOR_calculation(p):
    s = Solver()
    a = Int('s_a')
    b = Int('s_b')
    out = Int('s_out')
    circom_calculation = out == ((((a * b) % p + 1) % p - a) % p - b) % p

    cpp_parser = NORCircuitConstraintParser()
