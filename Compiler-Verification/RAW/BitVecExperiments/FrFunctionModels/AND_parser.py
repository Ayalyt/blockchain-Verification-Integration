...
class ANDCircuitConstraintParser:
    """简化电路约束解析器"""
    ...
    def parse_cpp_circuit(self):
        ...
        # 第1步: Fr_mul(&expaux[0], a, b)
        expaux0 = self.get_or_create_expaux(0)
        constraint1 = self.mul_gen.generate_fr_constraint(expaux0, a, b)
        self.constraints.append(("Fr_mul", "expaux[0] = a * b", constraint1))

        # 第2步: Fr_copy(out, expaux[0])
        constraint2 = self.copy_gen.generate_fr_constraint(out, expaux0)
        self.constraints.append(("Fr_copy", "out = expaux[0]", constraint2))
    ...
    def print_constraints(self):
        ...
        print(f"\n=== 电路逻辑 ===")
        print(f"实现逻辑: out = a * b")
        print(f"即: 简单的乘法运算")

        print(f"\n=== 变量映射 ===")
        print(f"输入变量: a, b")
        print(f"输出变量: out")
        print(f"中间变量: expaux[0]")


def main():
    """主函数：解析电路并输出约束"""
    parser = ANDCircuitConstraintParser()
