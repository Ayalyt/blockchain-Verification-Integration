...
class NANDCircuitConstraintParser:
    """包含常数的电路约束解析器"""
    ...
    def parse_cpp_circuit(self):
        ...
        // load src
        Fr_mul(&expaux[2],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 2]); // line circom 247
        Fr_sub(&expaux[0],&circuitConstants[0],&expaux[2]); // line circom 247
        // end load src
        ...
        # 第1步: Fr_mul(&expaux[2], a, b)
        expaux2 = self.get_or_create_expaux(2)
        constraint1 = self.mul_gen.generate_fr_constraint(expaux2, a, b)
        self.constraints.append(("Fr_mul", "expaux[2] = a * b", constraint1))

        # 第2步: Fr_sub(&expaux[0], circuitConstants[0], expaux[2])
        expaux0 = self.get_or_create_expaux(0)
        constraint2 = self.sub_gen.generate_fr_constraint(expaux0, const1, expaux2)
        self.constraints.append(("Fr_sub", "expaux[0] = 1 - expaux[2]", constraint2))

        # 第3步: Fr_copy(out, expaux[0])
        constraint3 = self.copy_gen.generate_fr_constraint(out, expaux0)
        self.constraints.append(("Fr_copy", "out = expaux[0]", constraint3))
    ...
    def print_constraints(self):
        ...
        print(f"1. expaux[2] = a * b         (乘法)")
        print(f"2. expaux[0] = 1 - expaux[2] (减法)")
        print(f"3. out = expaux[0]           (复制)")


def main():
    """主函数：解析电路并输出约束"""
    parser = NANDCircuitConstraintParser()
