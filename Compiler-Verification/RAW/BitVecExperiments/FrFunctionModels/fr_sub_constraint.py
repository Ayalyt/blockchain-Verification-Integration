...
class FrSubConstraint:
    ...
    def sub_l1ml2m(self, r: FrElement, a: FrElement, b: FrElement):
        ...
        # Montgomery减法: (a.longVal - b.longVal) % p
        diff_val = (a_dict['longVal'] - b_dict['longVal']) % self.p

        return And(
            r_dict['longVal'] == diff_val,
    ...
    def sub_l1ml2n(self, r: FrElement, a: FrElement, b: FrElement):
        ...
        # 然后进行Montgomery减法
        diff_val = (a_dict['longVal'] - b_montgomery.longVal) % self.p
    ...
    def sub_l1nl2m(self, r: FrElement, a: FrElement, b: FrElement):
        ...
        # 然后进行Montgomery减法
        diff_val = (a_montgomery.longVal - b_dict['longVal']) % self.p
    ...
    def sub_l1ns2(self, r: FrElement, a: FrElement, b: FrElement):
        ...
        # 然后进行普通减法
        diff_val = (a_dict['longVal'] - b_long_normal.longVal) % self.p
    ...
    def sub_s1l2n(self, r: FrElement, a: FrElement, b: FrElement):
        ...
        # 然后进行普通减法
        diff_val = (a_long_normal.longVal - b_dict['longVal']) % self.p
    ...
    def sub_s1s2(self, r: FrElement, a: FrElement, b: FrElement):
        ...
        # 短整数减法，检查是否溢出
        diff_short = a_dict['shortVal'] - b_dict['shortVal']

        # 如果溢出，使用长整数表示
        # 使用Z3的比较函数而不是Python的布尔运算
        overflow = Or(diff_short < BitVecVal(-2 ** 31, 32), diff_short >= BitVecVal(2 ** 31, 32))

        return If(overflow,
                  And(
                      r_dict['longVal'] == SignExt(224, diff_short),
                      r_dict['type'] == self.Fr_LONG,
                      r_dict['shortVal'] == 0
                  ),
                  And(
    ...
    def generate_fr_constraint(self, r: FrElement, a: FrElement, b: FrElement) -> ExprRef:
        ...
        # 组合条件
        sub_l1ml2m = And(a_l, a_m, b_l, b_m)
        sub_l1ml2n = And(a_l, a_m, b_l, b_n)
        sub_l1nl2m = And(a_l, a_n, b_l, b_m)
        sub_l1nl2n = And(a_l, a_n, b_l, b_n)
        sub_l1ms2m = And(a_l, a_m, b_s, b_m)
        sub_l1ms2n = And(a_l, a_m, b_s, b_n)
        sub_l1ns2 = And(a_l, a_n, b_s)
        sub_s1ml2m = And(a_s, a_m, b_l, b_m)
        sub_s1nl2m = And(a_s, a_n, b_l, b_m)
        sub_s1l2n = And(a_s, b_l, b_n)
        sub_s1s2 = And(a_s, b_s)

        # 组合所有条件
        return And(
            Implies(sub_l1ml2m, self.sub_l1ml2m(r, a, b)),
            Implies(sub_l1ml2n, self.sub_l1ml2n(r, a, b)),
            Implies(sub_l1nl2m, self.sub_l1nl2m(r, a, b)),
            Implies(sub_l1nl2n, self.sub_l1nl2n(r, a, b)),
            Implies(sub_l1ms2m, self.sub_l1ms2m(r, a, b)),
            Implies(sub_l1ms2n, self.sub_l1ms2n(r, a, b)),
            Implies(sub_l1ns2, self.sub_l1ns2(r, a, b)),
            Implies(sub_s1ml2m, self.sub_s1ml2m(r, a, b)),
            Implies(sub_s1nl2m, self.sub_s1nl2m(r, a, b)),
            Implies(sub_s1l2n, self.sub_s1l2n(r, a, b)),
            Implies(sub_s1s2, self.sub_s1s2(r, a, b))
        )

    def generate_ff_constraint(self, r: FrElement, a: FrElement, b: FrElement) -> ExprRef:
        """
        生成Fr_sub的FF_Constraint
...
def test_fr_sub():
    ...
    # 创建约束生成器
    constraint_gen = FrSubConstraint()
...
def test_specific_cases():
    ...
    solver.add(a.shortVal == 200)
    solver.add(b.shortVal == 100)
