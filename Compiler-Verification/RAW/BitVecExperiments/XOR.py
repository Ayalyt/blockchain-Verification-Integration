...
def run_XOR_constraint(p):
    ...
    circom_constraint = out % p == ((a + b) % p - ((2 * a) % p * b) % p) % p
    R1CS_constraint = ((a * 2) % p * (b * 1) % p) % p == (((out * -1) % p + (a * 1) % p) % p + (b * 1) % p) % p
...
def run_XOR_constraint_simplified(p):
    s = Solver()
    a = BitVec('a', 254)
    b = BitVec('b', 254)
    out = BitVec('out', 254)
    circom_constraint = out == (a + b - 2 * a * b) % p
    R1CS_constraint = (a * 2 * b * 1 - (out * 21888242871839275222246405745257275088548364400416034343698204186575808495616 + a * 1 + b * 1)) % p == 0
    implies_1 = Implies(circom_constraint, R1CS_constraint)
    implies_2 = Implies(R1CS_constraint, circom_constraint)
    target = And(implies_1, implies_2)
...
def run_XOR_calculation(p):
    s = Solver()
    a = Int('s_a')
    b = Int('s_b')
    out = Int('s_out')
    circom_calculation = out == ((a + b) % p - ((2 * a) % p * b) % p) % p

    cpp_parser = XORCircuitConstraintParser()
