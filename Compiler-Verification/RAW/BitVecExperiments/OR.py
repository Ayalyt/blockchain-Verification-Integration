...
from BitVecExperiments.FrFunctionModels.OR_parser import ORCircuitConstraintParser
from FrFunctionModels.XOR_parser import *


def run_OR_constraint(p):
    s = Solver()
    bw = p.bit_length() * 2
    ...
    circom_constraint = out % p == ((a + b) % p - (a * b) % p) % p
    R1CS_constraint = ((a * 1) % p * (b * 1) % p) % p == (((out * -1) % p + (a * 1) % p) % p + (b * 1) % p) % p
...
def run_OR_calculation(p):
    s = Solver()
    a = Int('s_a')
    b = Int('s_b')
    out = Int('s_out')
    circom_calculation = out == ((a + b) % p - (a * b) % p) % p

    cpp_parser = ORCircuitConstraintParser()
