...
host_inout_path = os.path.join(host_project_path, "SmartIFSyn-test", "compare", "time_test_2", "inout")

subprocess.run('sudo pkill -f "docker exec.*python3.*STV_time_test_2"', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

docker_command_test = (
    f'sudo docker run --name {container_name} --rm '
    f'-v {host_inout_path}:/inout 1994huxinwen/stc:latest '
    '/bin/bash -c "cd /code/solidity-type-checker && ./run.script 2>&1 | tee /inout/result.exp"'
)
...
sys.exit(exit_code)
# STV average time: 604.00172816872597
